document.addEventListener('DOMContentLoaded', (event) => {
  ...
  let config = {
    ...
    DENSITY_DISSIPATION: 0.6, // Keep this for stability
    VELOCITY_DISSIPATION: 0.1,
    PRESSURE_DISSIPATION: 0.7,
    PRESSURE_ITERATIONS: 50,
    CURL: 0,                 // ⬆️ Increased from 6 to add much more detail
    SPLAT_RADIUS: 0.0001,
  };
  ...
  void main () {
    ...
    // No Fresnel/vignette:
    // float fresnel = ...; vec3 spec = ...; color.rgb += spec * color.a;

    // Output straight alpha so it composites over ANY page background
    // via gl.blendFunc(SRC_ALPHA, ONE_MINUS_SRC_ALPHA)
    if (color.a > 0.001) {
