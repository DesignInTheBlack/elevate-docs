...
template: splash
hero:
  tagline: Stop Translating. Start Compiling.
  actions:
    - text: Learn More
...
Elevate isn't just another CSS framework - it's an actual compiler. The entire toolkit is ~100KB and generates zero CSS until you write utilities. No massive stylesheets. No unused code. Just a build tool that turns your design tokens into exactly the CSS you need.

Zero pre-built styles. Zero translation layer. Zero compromises.
...
Six months later, when you're debugging a layout issue at 11 PM, which one tells you why those specific values were chosen? Which one helps you understand the designer's intent?
pd:lg means "large padding from our spacing scale" - it connects directly to your design system's language. p-4 means "some arbitrary padding that seemed right at the time" - it's divorced from any design context and forces you to reverse-engineer the intent.
