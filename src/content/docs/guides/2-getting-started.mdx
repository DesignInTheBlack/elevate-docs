...
<div class="text:grey-2:snug:left">     

<!--
.text\:grey-2\:snug\:left {
color: #25262A;
line-height: 1.3;
text-align: left;
}
...
You can write `text:grey-2:left:snug` or `text:snug:grey-2:left` and the order doesn't matter. We'll get to the how and why later, but for now, just know that you can use any order you want.
</Aside>
...
```html
<div class="text:purple pd:d1 /md/ pd:d2 /lg/ @hover:[text:green_text:right]"> 
```
...
```html
<div class="@hover:[text:green_text:right_pd:d1]">  <!-- Hover state changes text and adds padding. -->
```
<br/>

Inside `@state:[...]`, use underscores instead of spaces. HTML class attributes split on spaces, so state blocks must be a single token (for example, `@hover:[text:green_text:right_pd:d1]`).

State chains use `+`, for example: `@hover+focus-visible:[bg-color:indigo_text:white]`.
...
Condense a full utility string into a single reusable class with `env:name`, then reopen it later with `env:name:open` to edit. Envelopes are a first-class way to create meaningful, reusable classes without leaving your HTML.

<br/>

<LinkCard title="Learn Envelopes" href="/guides/4-env-aliases/" />

<br/>

### Recommended Ordering

When multiple features are used together, keep your class list in this order:

`ctx -> env -> /breakpoint/ -> @state -> utilities`

This keeps scanning predictable and makes overrides easy to reason about. Property modifier order remains agnostic within a utility.
...
```html
<div class="ctx:card env:card /md/ @hover+focus-visible:[bg-color:indigo_text:white] pd:d4 bg-color:indigo-3 text:white shadow:soft">
  Card
</div>
```

If you need to edit the envelope, swap `env:card` for `env:card:open`, make your changes, then remove `:open` to close it again.
...
4. **This Is Elevate CSS** (after defining the custom `brand` property + tokens above):
...
### Isolating Elevate's Runtime

<br/>

In certain circumstances, you may want to run Elevate in a separate process to avoid conflicts with your project's dependencies. This is particularly useful when using Elevate alongside frameworks like Next.js, where dependency conflicts or module resolution issues may occur.
This can be done by creating a separate package.json file in the `elevate` directory and then running `npm start` from within that directory. Note that you can change your preferred command in the Elevate specific json file if you so choose.

```json
{
    "type": "module",
    "scripts": {
      "start": "set NODE_PATH=../node_modules && tsx core/index.ts"
    }
  }
```

If you encounter module resolution errors in the main setup while using Elevate with a framework like Next.js, try the isolated runtime approach first before further debugging dependency conflicts.
