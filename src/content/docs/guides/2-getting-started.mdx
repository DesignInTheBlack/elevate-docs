...
```html
<div class="text:purple pd:d1 /md/ pd:d2 /lg/ @hover:[text:green text:right]"> 
```
...
```html
<div class="@hover:[text:green text:right pd:d1]">  <!-- Hover state changes text and adds padding. -->
```
<br/>

You can chain property modifier combinations inside the contextual flag's brackets using spaces. Underscores are still accepted for backwards compatibility. This allows more concise expression of design intent. No more pseudo-class spam or unnecessary nesting!

State chains use `+`, for example: `@hover+focus-visible:[bg-color:indigo text:white]`.
...
```html
<div class="ctx:card env:card /md/ @hover+focus-visible:[bg-color:indigo text:white] pd:d4 bg-color:indigo-3 text:white shadow:soft">
  Card
</div>
```
