...
1. Download the Elevate CSS alpha from GitHub <Code code={`git clone https://github.com/DesignInTheBlack/Elevate.git`} title={`Terminal`}/>

2. Install Elevate's Dependencies <Code code={`npm install`} title={`Terminal`}/>

3. Watch Files for Changes and Convert Elevate's Syntax to CSS <Code code={`npm start`} title={`Terminal`}/>
